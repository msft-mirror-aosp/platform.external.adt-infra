...
@pytest.mark.vulkan_apps
@pytest.mark.async_timeout(30)
async def test_run_vulkan_samples(vulkan_samples_app, avd, get_screenshot):
    """Verifies that the vulkan samples app is running."""
    wait_time_s = 10
    await asyncio.sleep(wait_time_s)
    assert await vulkan_samples_app.is_running(), f"Vulkan Samples application process not found after {wait_time_s} seconds"
    assert avd.is_alive()
    await get_screenshot()