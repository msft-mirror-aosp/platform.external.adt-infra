...
def test_zip_file_contains_all_expected_files(emulator: BaseEmulator):
    ...
    act_files = [f.name for f in os.scandir(emu_root) if f.is_file()]
    ...
    act_sub_dirs = [f.name for f in os.scandir(emu_root) if f.is_dir()]
    ...
    for folder in exp_sub_dirs:
        ...
        sub_dir_files = [f.name for f in os.scandir(os.path.join(emu_root, folder))]
...
def get_exp_sub_dirs():
    ...
    exp_sub_dirs = {
        ...
        "lib": [
            ...
            "snapshot.proto",
            "ca-bundle.pem",
            "emulated_bluetooth_packets.proto",
            "hardware-properties.ini",
            "snapshot_service.proto",
            "cmake",
            "emulated_bluetooth_vhci.proto",
            "hostapd.conf",
    ...
    if platform.system() == "Windows":
        ...
        exp_sub_dirs["lib64"].update(lib64_linux)
        return exp_sub_dirs
    elif platform.system() == "Darwin":
        if platform.processor() == "arm":
            exp_sub_dirs["qemu"] = ["darwin-aarch64"]
            exp_sub_dirs["_CodeSignature"] = ["CodeResources"]
            return exp_sub_dirs
        else:
