...
from acloud.internal.lib.android_build_client import AndroidBuildClient
from acloud.internal.lib.android_compute_client import AndroidComputeClient
...
class GCEDevice:
    ...
    ADB_PORT = 5555
    VNC_PORT = 8556  # Piggy backed as gRPC port right now.
    ...
    def __init__(self, config_file, image, build_id, adb, base_img=None, gpu=None):
        self.launch = None
        self.adb_port = None
        self.grpc_port = None
        self.image = image
    ...
    def _configure(self, config_file):
        ...
        if self.base_img:
            self.cfg.stable_goldfish_host_image_name = self.base_img
    ...
    def _get_adb_keys(self):
        ...
        scp_args = GCEDevice.SCP_ARGS % {
            ...
            "remote_file": "/opt/emulator/adbkey",
    ...
    def _forward_ports(self):
        ...
        self.adb_port = fwd.adb_port
        self.grpc = fwd.vnc_port
        logging.info("Forwarded adb on: %d, and grpc on: localhost:%d", self.adb_port, self.grpc)
    ...
    def get_grpc_url(self):
        ...
        return "localhost:{}".format(self.grpc)
    ...
    def boot_complete(self):
        ...
        return self.is_running() and "1" in self.adb.cmd(
            ["shell", "getprop", "sys.boot_completed"], self.get_adb_port()
        )

    def available(self):
        return self.is_running() and self.adb.cmd(
            ["wait-for-device"], self.get_adb_port()
        )
    ...
    def delete(self):
        ...
        return device_driver.DeleteAndroidVirtualDevices(
            self.cfg, [self.instance_name()]
        )

    def __str__(self):
        return "%s - Running: %s, Booted: %s" % (
            self.image,
            self.is_running(),
            self.status,
        )
