#!/usr/bin/env python3
"""
Identity Credential Authentication Test (SECURITY section)
Prerequisites:
...
UI flow:
  ...
  2. Launch and navigate to 'Identity Credential Authentication'.
...
TEST_NAME = "Identity Credential Authentication"


def main():
    args = parse_security_args(
        "Run CTS Verifier Identity Credential Authentication Test."
    )
    setup_and_navigate(
        TEST_NAME,
        prereq_state=PrereqState.PIN_AND_FINGERPRINT,
        skip_prereqs=args.skip_prereqs,
        screenshot_subdir="security_identity_credential_authentication",
    )
    dismiss_initial_dialog()
