#!/usr/bin/env python3
"""
Fingerprint Bound Keys Test (SECURITY section)
Prerequisites:
...
UI flow:
  ...
  2. Launch and navigate to 'Fingerprint Bound Keys Test'.
  3. Dismiss initial instruction/permission dialogs if present.
  4. Find and tap 'Start Test' button.
  5. Wait for fingerprint authentication prompt and simulate touch.
...
TEST_NAME = "Fingerprint Bound Keys Test"


def main():
    args = parse_security_args("Run CTS Verifier Fingerprint Bound Keys Test.")
    setup_and_navigate(
        TEST_NAME,
        prereq_state=PrereqState.PIN_AND_FINGERPRINT,
        skip_prereqs=args.skip_prereqs,
        screenshot_subdir="security_fingerprint_bound_keys",
    )
    dismiss_initial_dialog()
