#!/usr/bin/env python3
"""
Sharesheet Payload Toggle Test
Taps Share to open the chooser, selects all 3 images (by tapping each
"Selectable image" item), then taps the CTS Verifier icon to send the
selected images. The test auto-passes on completion.

Flow:
  1. setup() + navigate_to "Sharesheet Payload Toggle Test"
  2. Dismiss OK if present
  3. Tap Share button
  4. Chooser opens with 3 selectable images + CTS Verifier icon at the bottom
  5. Tap all 3 "Selectable image" items
  6. Tap the CTS Verifier LinearLayout (clickable container near bottom-left)
  7. Test auto-passes and returns to main list
  8. Export ZIP and verify test_result.xml

Screenshots saved to ./cts_screenshots/sharesheet_pt/<step>_<desc>.png
"""
...
TEST_NAME  = "Sharesheet Payload Toggle Test"
set_screenshot_dir("sharesheet_payload_toggle_test")
...
# Tap CTS Verifier container (share target)
root = ui_dump()
