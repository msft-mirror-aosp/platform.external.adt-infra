...
"""Authentic CTS Verifier Bluetooth Classic Secure Client/Server Test Automation.

Coordinated dual-device test over Netsim RF virtual medium with Bluetooth pairing:
1. Companion Node (emulator-5556) launches SecureServerActivity & makes itself discoverable.
2. DUT Node (emulator-5554) launches SecureClientActivity & scans for nearby devices.
3. Authenticates & pairs both nodes via Secure Simple Pairing (SSP) / PIN prompts.
4. Bidirectional exchange of 10 encrypted messages verified over secure RFCOMM socket.
...
from mesh_cts_common import (
    ...
    find_any_node,
    find_device_item,
    find_dialog_confirmation_button,
...
TEST_NAME = "Secure Client"
CLIENT_ACTIVITY = "com.android.cts.verifier.bluetooth.SecureClientActivity"
SERVER_ACTIVITY = "com.android.cts.verifier.bluetooth.SecureServerActivity"


def main():
    setup_mesh_nodes(DUT_SERIAL, COMPANION_SERIAL)
    set_screenshot_dir("bluetooth_classic_secure_test")
    ...
    with mesh_diagnostics(TEST_NAME, [DUT_SERIAL, COMPANION_SERIAL]):
        try:
            ...
            tap_on(DUT_SERIAL, target_node, sleep_after=1.0)

            # 4. Monitor secure RFCOMM data stream and Pass condition on DUT
            print(
                f"Step 4: Monitoring secure RFCOMM data stream and Pass condition on DUT ({DUT_SERIAL})..."
            )
            ...
            screenshot("01_secure_rfcomm_passed")
            ...
            if pass_btn is None or pass_btn.attrib.get("enabled") != "true":
                raise TimeoutError(
                    f"DUT ({DUT_SERIAL}): RFCOMM test did not complete — Pass button was not enabled"
                )
            ...
            print(
                ">>> SUCCESS: Bluetooth Classic Secure Client/Server test passed and verified!"
            )
