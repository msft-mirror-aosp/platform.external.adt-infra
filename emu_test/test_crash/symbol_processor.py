...
class SymbolProcessor(object):
    """A Breakpad symbol processor that can translate your minidup into a readable stacktrace."""
    ...
    def decode_stack_trace(self, dmpfile):
        ...
        logging.debug("Run %s", " ".join([self.stackwalk, dmpfile, self.symbol_dir]))
        with open(os.devnull, "w") as devnull:
            self.decoded = subprocess.check_output(
                [self.stackwalk, dmpfile, self.symbol_dir], stderr=devnull
            )
        return self.decoded
