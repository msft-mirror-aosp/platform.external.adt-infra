...
class HelpTest(testcase_base.BaseConsoleTest):
  ...
  def test_help_command_noauth(self):
  ...
  def test_help_command_auth(self):
    ...
    Test steps:
      1. Launch an emulator avd
      2. Copy the auth_token value from ~/.emulator_console_auth_token
      3. Run: auth auth_token
      4. Run: help, and verify 1
    Verify:
      1. crash, kill, redir, power, event, avd ,finger, geo, sms, cdma,
