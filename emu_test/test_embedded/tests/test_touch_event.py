...
def test_touch_event_identifier_ranges(at_home):
    ...
    stub.sendTouch(TouchEvent(touches=[Touch(x=203, y=823, identifier=-124543)]))
    stub.sendTouch(TouchEvent(touches=[Touch(x=801, y=1281, identifier=-23122)]))


@pytest.mark.e2e
@pytest.mark.timeout(timeout=10, func_only=True)
def test_touch_event_identifier_to_many(at_home):
