...
public class SettingsCameraPermissionsWatcher implements UiWatcher {
    ...
    public SettingsCameraPermissionsWatcher(UiDevice device) {
        this.mDevice = device;
    }
    ...
    public boolean checkForCondition() {
        ...
        boolean hasPopup =
                mDevice.findObject(new UiSelector().textMatches("(?i)next(?-i)"))
                        .waitForExists(TimeUnit.MILLISECONDS.convert(3L, TimeUnit.SECONDS));
        try {
            if (hasPopup) {
                mDevice.findObject(new UiSelector().textMatches("(?i)next(?-i)")).click();
                condition = true;
            }
        }
