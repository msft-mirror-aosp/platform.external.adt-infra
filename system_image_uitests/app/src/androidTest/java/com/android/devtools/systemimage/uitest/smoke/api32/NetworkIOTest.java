...
public class NetworkIOTest {
    ...
    public void toggleCellularDataMode() throws Exception {
        ...
        if (!(SettingsUtil.navigateToSettingsPath(device, "Network & internet", "SIMs"))) {
        ...
        if (!isCellularDataEnabled) {
            assertTrue("Mobile data switch not enabled.", SettingsUtil.clickAndConfirmSwitch(device, mobileDataSwitch));
        ...
        assertTrue("Mobile data switch not disabled.", SettingsUtil.clickAndConfirmSwitch(device, mobileDataSwitch));
        try {
            scrollable.scrollIntoView(dataWarning);
        ...
        assertTrue("Mobile data switch not reset.", SettingsUtil.clickAndConfirmSwitch(device, mobileDataSwitch));
        try {
            scrollable.scrollIntoView(dataWarning);
    ...
    /**
     * Toggles the airplane mode on and off.
     * <p>
     * This method first checks if the airplane mode is already enabled. If it is, it navigates to the airplane mode setting
     * and disables it. Then it enables the airplane mode, verifies that it is enabled, disables it again, and finally verifies
     * that it is disabled.
     *
     * @throws Exception if there is an issue with the UI Automator or if the airplane mode setting cannot be found or toggled.
     */
    private void toggleAirplaneMode() throws Exception {
        ...
        if (!(SettingsUtil.navigateToSettingsPath(device, "Network & internet"))) {
        ...
        if (NetworkUtil.isAirplaneModeEnabled_v3(device, airplaneModeIcon)) {
            SettingsUtil.navigateToSettingsPath(device, "Network & internet", "Airplane mode");
        }
        assertFalse("Airplane mode is not disabled at the start.",
        ...
        SettingsUtil.navigateToSettingsPath(device, "Network & internet", "Airplane mode");
        assertFalse("Airplane mode is not disabled at the end.",
                NetworkUtil.isAirplaneModeEnabled_v3(device, airplaneModeIcon));
    }

    /**
     * Test method to verify that the airplane mode can be enabled and disabled.
     * <p>
     * This method calls the {@link #toggleAirplaneMode()} method to enable and disable the airplane mode.
     *
     * @throws Exception if there is an issue with the UI Automator or if the airplane mode setting cannot be found or toggled.
     */
    @Test
    ...
    /**
     * Test method to stress test the enabling and disabling of the airplane mode.
     * <p>
     * This method calls the {@link #toggleAirplaneMode()} method four times to repeatedly enable and disable the airplane mode.
     *
     * @throws Exception if there is an issue with the UI Automator or if the airplane mode setting cannot be found or toggled.
     */
    @Test
    ...
    public void togglePreferredNetworkType() throws Exception {
        ...
        if (!(SettingsUtil.navigateToSettingsPath(device, "Network & internet", "SIMs", "Preferred network type"))) {
            fail("Failed to navigate to Network & internet > SIMs settings > Preferred network type.");
        }
        SettingsUtil.clickAndConfirmSwitch(device, dataSwitchLTE);

        SettingsUtil.navigateToSettingsPath(device, "Network & internet", "SIMs", "Preferred network type");
        SettingsUtil.clickAndConfirmSwitch(device, dataSwitch3G, dataSwitchLTE);

        SettingsUtil.navigateToSettingsPath(device, "Network & internet", "SIMs", "Preferred network type");
        SettingsUtil.clickAndConfirmSwitch(device, dataSwitch2G, dataSwitch3G);

        SettingsUtil.navigateToSettingsPath(device, "Network & internet", "SIMs", "Preferred network type");
        assertTrue("LTE switch not reset.", SettingsUtil.clickAndConfirmSwitch(device, dataSwitchLTE, dataSwitch2G));
    }
}
