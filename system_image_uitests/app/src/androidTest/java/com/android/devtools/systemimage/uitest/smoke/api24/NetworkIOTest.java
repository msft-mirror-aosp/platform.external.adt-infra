...
import com.android.devtools.systemimage.uitest.utils.api24.NetworkIOTestUtils;
import com.android.devtools.systemimage.uitest.utils.api24.NetworkUtil;
...
public class NetworkIOTest {
    ...
    public void enableAirplaneMode() throws Exception {
        ...
        if (NetworkUtil.isAirplaneModeEnabled(device)) {
            AppLauncher.launchPath(instrumentation, path);
            NetworkIOTestUtils.toggleAirplaneMode(device);
        }
    ...
    public void disableDataRoaming() throws Exception {
        ...
        if (!NetworkIOTestUtils.isDataRoamingEnabled(context)) {
            NetworkIOTestUtils.toggleRoaming(device);
        }

        assertTrue("Data roaming is not enabled.", NetworkIOTestUtils.isDataRoamingEnabled(context));

        // Disable data roaming option.
        NetworkIOTestUtils.toggleRoaming(device);

        assertFalse("Data roaming is not disabled.", NetworkIOTestUtils.isDataRoamingEnabled(context));
